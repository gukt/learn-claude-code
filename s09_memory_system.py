...
"""
# 记忆系统
你来自 s08 的智能体功能强大且可扩展。它可以安全地执行工具，可通过钩子进行扩展，并且借助上下文压缩支持长时间的会话。
但它有健忘症。每次你开启新会话时，智能体都是第一次见到你。
它不记得你更偏爱 pnpm 而非 npm，不记得你三次让它停止修改测试快照，也不记得遗留目录无法删除 —— 因为部署依赖于它。
你不得不在每个会话中反复重申。

解决办法是建立一个小型且持久的记忆存储 —— 并非存储智能体见过的所有内容，而是筛选出一组下次仍有意义的关键信息。

## 问题
没有记忆的话，新的会话就会从零开始。智能体会不断遗忘诸如用户长期偏好、你多次重复的纠正内容、代码本身未明确体现的项目约束，以及项目依赖的外部引用等信息。
最终得到的智能体总像是第一次见到你。你会浪费时间重新建立本应一次性保存并自动加载的上下文。

## 解决方案
一个小型的基于文件的内存存储会将持久化的事实保存为单独的 Markdown 文件，这些文件带有 YAML 前置内容（即每个文件顶部的元数据块，由---行分隔）。
在每次会话开始时，相关的记忆会被加载并注入到模型的上下文之中。
