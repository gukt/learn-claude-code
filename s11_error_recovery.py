...
大多数失败并非真正的任务失败 —— 它们是尝试不同路径的信号。

## 你将学到：
- 三类可恢复故障：截断、上下文溢出和瞬时传输错误
- 如何将每种故障路由到正确的恢复分支（继续、压缩或退避）
- 为何重试预算能避免无限循环
- 恢复状态如何让 “原因” 清晰可见，而非将其隐藏在异常处理块中

你的智能体此刻正在执行实际任务 —— 读取文件、编写代码、跨多轮调用工具。而实际工作必然会产生真实的失败。
输出在句子中间被截断，提示内容超出模型的上下文窗口，API 超时或触发速率限制。
如果每一次这类失败都直接终止运行，你的系统会显得十分脆弱，用户也会因此不再信任它。
但关键在于：大多数这类失败并非真正的任务失败，它们只是表明下一步需要换一种不同的延续路径。

## 问题
你的用户要求智能体重构一个大文件。模型开始编写新版本，但输出因达到 max_tokens 而在函数中间停止。
如果不进行恢复，智能体只会生成一个未完成的文件就停止运行。
用户必须察觉这一情况，重新发起提示，并寄希望于模型能从中断处继续生成。

或者：对话已进行 40 轮。累积的消息使提示超出了模型的上下文限制。API 返回错误。如果不进行恢复，整个会话都将丢失。
或者：短暂的网络故障导致连接中断。如果没有恢复机制，智能体就会崩溃，而同样的请求在一秒后就能成功。

这三种情况属于不同类型的故障，每种都需要采取不同的恢复措施。单一的通用重试机制无法正确处理这三种情况。

## 解决方案
先对故障进行分类，再选择恢复分支，并设置重试限制，避免系统无限循环。
