...
"""
## 引言
从 s06 开始，您的代理功能强大且寿命长。
它读取文件、编写代码、运行 shell 命令、委托子任务并压缩自己的上下文以继续运行。但是没有安全问题。
模型提出的每个工具调用都直接执行。要求它删除目录，它会 —— 没有问题。
在您授予该代理访问任何重要内容的权限之前，您需要在 “模型想要做 X” 和 “系统实际上做 X” 之间有一个网关（gate)。
...
s07_permission_system.py 是一个权限系统，它通过一个权限管道来检查每个工具调用。
权限管道有四个步骤：
1. 拒绝规则
2. 模式检查
3. 允许规则
...
- plan
- auto
这足够构建一个真正的、可理解的权限系统，而不会让读者在第一天就被所有的高级策略分支淹没。

关键洞察：“安全是一个管道，而不是一个布尔值。”
"""
import json
import os
import re
...
# -- Bash 安全验证器 --
# 验证 Bash 命令是否存在明显危险的模式。
# 教学版本故意保持这个简单易读，
# 先抓住几个高风险模式，然后让权限管道决定是否拒绝或询问用户。
class BashSecurityValidator:
    ...
    def describe_failures(self, command: str) -> str:
        ...
        # 如果没有错误，返回：No issues detected
        # 否则，返回：Security flags: 错误名称 (模式: 模式) 的拼接字符串
        if not failures:
...
# 教学版本故意保持决策路径短，以便读者可以在添加更多高级策略层之前自己实现它。
class PermissionManager:
    ...
    def __init__(self, mode: str = "default", rules: list = None):
        ...
        # 简易拒绝行为追踪有助于及时发现智能代理反复请求系统不允许执行的操作的情况。
        # 连续拒绝的计数器
        # 最大连续拒绝次数
...
# -- Tool handlers --
# 工具处理程序映射
# 本教学实例只提供 4 个常用的基本命令：bash，read_file，write_file，edit_file
TOOL_HANDLERS = {
...
# 系统提示词：
# 你是一个 Coding Agent,工作在 {WORKDIR} 目录下，使用工具来解决问题。
# 用户控制权限。有些工具调用可能被拒绝。
...
# 试一试：
# Start in default mode -- every write tool asks for approval
# 以 default 模式启动 —— 所有写入工具均需请求批准

# Try plan mode -- all writes are blocked, reads pass through
# 尝试 plan 模式 —— 所有写入操作均被阻止，读取操作可正常通过

# Try auto mode -- reads auto-approved, writes still ask
# 尝试 auto 模式 —— 读取操作自动批准，写入操作仍需请求

# Answer "always" to permanently allow a tool
# 回答 “始终” 以永久允许某个工具

# Type /mode plan to switch modes at runtime
# 输入 /mode plan 可在运行时切换模式

# Type /rules to inspect current rule set
# 输入 /rules 以查看当前规则集
